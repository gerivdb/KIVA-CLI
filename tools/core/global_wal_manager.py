...
class GlobalWALManager:
    ...
    def _init_db(self):
        ...
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_events_ecosystem ON events(ecosystem_id)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_events_type ON events(event_type)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_operations_event ON operations(event_id)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_operations_repo ON operations(repository)")
    ...
    def append_event(
        ...
        with self._lock:
            event_id = self._generate_id(f"event_{ecosystem_id}_{datetime.utcnow().isoformat()}")
            intent_hash = self._generate_intent_hash(event_id, event_type.value, repositories, parent_intent_hash)
            
            phi_cps_delta = phi_cps_current - phi_cps_baseline
            phi_cps_alert = abs(phi_cps_delta) > phi_cps_threshold
            
            validation_state = ValidationState.PENDING if phi_cps_alert else ValidationState.SUCCESS
    ...
    def add_operation(
        ...
        with self._lock:
            operation_id = self._generate_id(f"op_{event_id}_{repository}_{datetime.utcnow().isoformat()}")
            ...
            """, (
                operation_id, event_id, operation_type, repository,
                path, commit_sha, status.value, duration_ms, error_message
            ))
    ...
    def add_dependency(
        ...
        with self._lock:
            dependency_id = self._generate_id(f"dep_{from_repo}_{to_repo}_{datetime.utcnow().isoformat()}")
    ...
    def perform_rollback(
        ...
        with self._lock:
            rollback_id = self._generate_id(f"rollback_{event_id}_{datetime.utcnow().isoformat()}")
            ...
            """, (
                rollback_id, event_id, datetime.utcnow().isoformat(), reason,
                json.dumps(commits_reverted), phi_cps_before, phi_cps_after, int(success)
            ))
    ...
    def get_statistics(
        ...
        # Events by type
        cursor.execute(f"SELECT event_type, COUNT(*) FROM events {where_clause} GROUP BY event_type", params)
        events_by_type = {row[0]: row[1] for row in cursor.fetchall()}
        
        # Events by severity
        cursor.execute(f"SELECT severity, COUNT(*) FROM events {where_clause} GROUP BY severity", params)
        events_by_severity = {row[0]: row[1] for row in cursor.fetchall()}
        
        # φ-CPS alerts
        cursor.execute(f"SELECT COUNT(*) FROM events {where_clause} AND phi_cps_alert = 1", params)
        phi_cps_alerts = cursor.fetchone()[0]
        
        # Rollbacks
        cursor.execute(f"SELECT COUNT(*) FROM events {where_clause} AND rollback_performed = 1", params)
        ...
        # Success rate
        cursor.execute(f"SELECT validation_state, COUNT(*) FROM events {where_clause} GROUP BY validation_state", params)
